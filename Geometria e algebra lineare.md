...
- #TEOREMA(4). **Teorema di Rappresentazione**
	...
	*Dimostrazione*:
	...
	$\mathcal{L}(x)=\mathcal{L}\left( \sum_{j=1}^n x_{j}\mathbf{u}_{j}\right)=\sum_{j=1}^n x_{j}\mathcal{L}(\mathbf{u}_{j})=\sum_{j=1}^n x_{j}\sum_{i=1}^m a_{ij}\mathbf{v}_{i}=\sum_{j=1}^n\sum_{i=1}^m a_{ij}$