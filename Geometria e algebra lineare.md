...
quindi la matrice $\mathbf{A}$ si può scrivere nei modi seguenti: $\mathbf{A}=\begin{pmatrix}\mathbf{a}_{1}\\ \mathbf{a}_{2}\\ \vdots \\ \mathbf{a}_{m}\end{pmatrix}=(\mathbf{a}^1,\mathbf{a}^2),\cdots,$